...
from alita_app.config import DEFAULT_PROFILES_DIRECTORY as DEFAULT_PROFILES_PATH  # noqa: F401

# Import config to ensure .env is loaded before reading REACHY_MINI_CUSTOM_PROFILE
from alita_app.config import config  # noqa: F401
from alita_app.tools.tool_constants import SystemTool


if TYPE_CHECKING:
    from alita_app.tools.background_tool_manager import BackgroundToolManager


logger = logging.getLogger(__name__)


DEFAULT_PROFILES_MODULE = "alita_app.profiles"
...
class Tool(abc.ABC):
    ...
    def spec(self) -> Dict[str, Any]:
        """Return the tool spec for Anthropic Messages API consumption."""
        return {
            "name": self.name,
...
def _load_profile_tools() -> None:
    ...
    for tool_name in tool_names:
        ...
        if not loaded:
            shared_module_path = f"alita_app.tools.{tool_name}"
            try:
                source = _try_load_tool(
                    tool_name,
