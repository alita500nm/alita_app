...
class MovementManager:
    ...
    def __init__(
        ...
        self._hibernating = False  # true when state machine is in HIBERNATE
    ...
    def set_hibernating(self, hibernating: bool) -> None:
        """Enable or disable hibernation mode.

        While hibernating:
        - Breathing is suppressed.
        - Queued moves are cleared.
        - The robot holds its current (or neutral) pose.

        Thread-safe: the change is posted to the worker command queue.
        """
        self._command_queue.put(("set_hibernating", hibernating))
    ...
    def _handle_command(self, command: str, payload: Any, current_time: float) -> None:
        ...
        if command == "queue_move":
            ...
            if desired_state:
                ...
                self._antenna_unfreeze_blend = 0.0
            self.state.update_activity()
        elif command == "set_hibernating":
            self._hibernating = bool(payload)
            if self._hibernating:
                # Clear all moves and stop breathing
                self.move_queue.clear()
                self.state.current_move = None
                self.state.move_start_time = None
                self._breathing_active = False
                logger.info("MovementManager: hibernation ON — cleared moves, breathing suppressed")
            else:
                logger.info("MovementManager: hibernation OFF — movement enabled")
        else:
