        ...
        while not self._stop_event.is_set():
            try:
                ...
                if frame is not None:
                    ...
                    with self.frame_lock:
                        if self.latest_frame is None:
                            logger.info("DIAG camera_worker: first frame shape=%s dtype=%s", frame.shape, frame.dtype)
                ...
                if frame is None:
                    ...
                    if self._none_count == 1:
                        logger.warning("DIAG camera_worker: first None from get_frame()")
                    elif self._none_count == 50:
                        logger.warning("DIAG camera_worker: 50 consecutive Nones from get_frame()")
